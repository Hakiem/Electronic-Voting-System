...
<?php
	...
	DrawHeader('VOTERS THAT HAVE ACCESSED THEIR E-BALLOT PAPER, NOT YET CASTED');
	$result = mysql_query('SELECT * FROM votestudents WHERE HasGotCode = 1 AND HasVoted = 0 AND AccessedBallotPaper = 1');
?>
	...
	<tr>
		...
		<div style="background-image:url(Images/Tackle.png); padding:5px;font-size:13px; height:20px; font-weight:bold; color:Red;text-align:center;">
			<?php 
				echo 'VOTERS THAT HAVE ACCESSED THEIR E-BALLOT PAPER, NOT YET CASTED (TOTAL : '.mysql_num_rows($result).')';
			?>
		</div>
