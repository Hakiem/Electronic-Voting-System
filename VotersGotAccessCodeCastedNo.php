...
<?php
	...
	DrawHeader('VOTERS THAT HAVE GOT ACCESS-CODE BUT HAVE NOT YET CASTED THEIR VOTE');
	$result = mysql_query('SELECT * FROM votestudents WHERE HasGotCode = 1 AND HasVoted = 0 AND AccessedBallotPaper = 0');
?>
	...
	<tr>
		...
		<div style="background-image:url(Images/Tackle.png); padding:5px;font-size:13px; height:20px; font-weight:bold; color:Red;text-align:center;">
			<?php 
				echo 'VOTERS THAT HAVE GOT ACCESS-CODE BUT HAVE NOT YET CASTED THEIR VOTE (TOTAL : '.mysql_num_rows($result).')';
			?>
		</div>
		<!-- Start ContentPlaceHolder -->
			<?php
				if(mysql_num_rows($result) > 0)
				{
					...
					echo '<th>_</th><th>VOTER\'S REGISTRATION NO.</th><th>VOTER\'S NAME.</th><th>VOTER\'S GENDER</th><th>TIME GOT CODE</th>';
					while($row = mysql_fetch_row($result))
					{
						...
						echo '<tr bgcolor='.$bgc.'>';
							...
							echo '<td style=width:35%>&nbsp;&nbsp;'.$row[1].'</td>';
