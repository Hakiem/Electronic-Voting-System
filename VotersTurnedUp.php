...
<?php
	...
	DrawHeader('VOTERS THAT HAVE TURNED UP');
	$result = mysql_query("SELECT * FROM votestudents WHERE HasGotCode = 1");
...
?>
	...
	<tr>
		...
		<div style="background-image:url(Images/Tackle.png); padding:5px;font-size:13px; height:20px; font-weight:bold; color:Red;text-align:center;">
			<?php 
				echo 'VOTERS THAT HAVE TURNED UP (RATIO : '.mysql_num_rows($result).' / '.$K[0].')';
			?>
		</div>
		<!-- Start ContentPlaceHolder -->
			<?php
				if(mysql_num_rows($result) > 0)
				{
					...
					echo '<h6>THERE ARE NO VOTERS VOTERS THAT HAVE TURNED UP YET</h6><hr>';
			?>
